...
namespace TelegramBot.Tools.APIServices.Services
{
    public class ReviewsApiService
    {
        ...
        public ReviewsApiService(HttpService service)
        {
            _httpService = service;
        }

        public async Task<List<ReviewListDto>> GetAllAsync(int page)
            => await _httpService.GetAsync<List<ReviewListDto>>($"{_baseUrl}?page={page}");
